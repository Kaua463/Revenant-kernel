...
flash_boot() {
  ...
  mkdir -p "$AKHOME/verify-repack" || abort "Cannot create verification directory.";
  cd "$AKHOME/verify-repack";
  magiskboot unpack "$AKHOME/boot-new.img" || abort "Repacked boot cannot be unpacked.";
  cmp -s kernel "$AKHOME/Image" || abort "Repacked kernel differs from candidate.";
  [ ! -s ramdisk.cpio ] || abort "Unexpected boot ramdisk.";
  cd "$AKHOME";
  # Pad the regular file first; never clear a partition before replacement is ready.
  truncate -s 67108864 boot-new.img || abort "Cannot pad boot image.";
  new_hash=$(sha256sum boot-new.img | awk '{print $1}');
  blockdev --setrw "$BLOCK" || abort "Cannot enable boot writes.";
  dd if=boot-new.img of="$BLOCK" bs=1048576 conv=fsync || abort "Boot write failed. Restore stock via fastboot before reboot.";
  sync;
  written_hash=$(sha256sum "$BLOCK" | awk '{print $1}');
  [ "$written_hash" = "$new_hash" ] || abort "Boot readback mismatch. Restore stock via fastboot before reboot.";
  ui_print "Boot readback SHA-256 verified.";
}
