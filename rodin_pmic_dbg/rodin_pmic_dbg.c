...
// Original authors: Wen Su, Jeter Chen (MediaTek)
// Repackaged for Revenant/rodin (POCO X7 Pro / MT6899 / Dimensity 8400)
// from Mayuri-Chan/MTK_kernel_device_modules_6.6 drivers/mfd/mtk-spmi-pmic-debug.c
//
// Purpose: standalone .ko that probes the DTS node already present on rodin
// (compatible = "mediatek,spmi-pmic-debug") and exposes a sysfs pmic_access
// attribute for read/write access to ANY SPMI PMIC slave register on this SoC.
//
// On rodin the relevant slave is mt6319@8 (VPROC_prime, 4-phase buck).
// Stock HyperOS ships this DTS slot WITHOUT a matching .ko, so the sysfs
// node never appears. Loading this module fills the slot.
//
// USAGE (after insmod):
//   echo "0x1449" > .../pmic_access   # arm read of register 0x1449
//   cat .../pmic_access               # last read value (hex)
//   echo "0x1449 0x5C" > .../pmic_access  # write 0x5C to 0x1449 on local slave
//   echo "0x81449 0x5C" > .../pmic_access # same on slave 8 (mt6319)
//
// Read is always safe. Writes go to PMIC via SPMI bus immediately.
